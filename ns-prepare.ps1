...
# Тривалість ≈ 20 хв на номер. Наприкінці стан номера -> review (або ready, якщо чисто й є паспорт року).
...
if ($curState -in @("", "scanned", "accepted", "review", "fix")) {
    $newState = if ($nFlags -eq 0 -and $hasPassport) { "ready" } else { "review" }
    $why = if ($nFlags -gt 0) { "позначок: $nFlags" } elseif (-not $hasPassport) { "рік $($man.year) без паспорта" } else { "чисто" }
    Set-NsIssueState -IssueDir $issueDir -Manifest (Read-NsManifest -IssueDir $issueDir) -State $newState -Note "ns-prepare: $why"
    Write-Log ("    стан номера: {0} ({1})" -f $newState, $why) Gray
